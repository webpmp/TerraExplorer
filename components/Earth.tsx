...
const UniversalMarker: React.FC<{ 
  ...
  return (
    <group position={position} onClick={onClick} ref={meshRef}>
      ...
      {/* Border Outline (Inverted Hull) */}
      <mesh scale={[1.4, 1.4, 1.4]} pointerEvents="none">
           <sphereGeometry args={[size, 16, 16]} />
           <meshBasicMaterial 
...
const RotatingEarth = forwardRef<THREE.Mesh, EarthProps>((props, ref) => {
  ...
  const processedMarkers = useMemo(() => {
    ...
    return itemsWithPos.map((item, i, arr) => {
        ...
        // Logic: overlapping radius = (size1 + size2).
        // If distance < (size1 + size2), they overlap.
        // We conservatively assume size2 ~ size1 for simple scaling.
        // We want finalRadius * 2 < minDist approximately.
    ...
    <group ref={groupRef}>
      ...
      {!isGreen && (
        <mesh scale={[1.2, 1.2, 1.2]}>
            ...
            <meshBasicMaterial
                color={isModern ? "#64b5f6" : retroColor}
                transparent
                opacity={isModern ? 0.15 : 0.05}
                side={THREE.BackSide}
                blending={THREE.AdditiveBlending}
            />
        </mesh>
