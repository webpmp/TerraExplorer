...
const adapter = `
function adaptLocationInfoToResolvedEntity(loc: any): ResolvedEntity {
    if (!loc) return loc;
    ...
    const meta = createMetadata({
        ...
        climate: loc.climate ? { value: loc.climate.name, description: loc.climate.description, provenance: { stage: 'recovery', source: 'ai', timestamp: new Date().toISOString() } as any } : undefined,
        population: loc.population?.current ? { value: loc.population.current.value || null, status: 'available', provenance: { stage: 'recovery', source: 'ai', timestamp: new Date().toISOString() } as any } : undefined,
